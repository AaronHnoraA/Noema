    ...
    (condition-case nil
        (if (not (stringp value))
          ...
          (let* ((project-root
                 ...
                 ;; Canonicalize an absolute path preview before prefix
                 ;; replacement.  On macOS the same file is routinely named
                 ;; through /tmp or /var while `file-truename' returns
                 ;; /private/tmp or /private/var.  Comparing one spelling to
                 ;; the other would miss $PROJECT redaction.
                 (preview-value
