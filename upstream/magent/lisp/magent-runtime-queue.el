...
(defvar magent-runtime-queue--submission-samplers
  (make-hash-table :test #'eq :weakness 'key)
  "Request-local LLM samplers keyed by runtime submission identity.")
...
(defun magent-runtime-submission-sampler (submission)
  "Return the request-local LLM sampler captured for SUBMISSION."
  (gethash submission magent-runtime-queue--submission-samplers))

(defun magent-runtime-submission-set-sampler (submission sampler)
  "Associate callable SAMPLER with SUBMISSION and return SAMPLER."
  (if sampler
      (progn
        (unless (functionp sampler)
          (error "Runtime submission sampler is not callable: %S" sampler))
        (puthash submission sampler
                 magent-runtime-queue--submission-samplers))
    (remhash submission magent-runtime-queue--submission-samplers))
  sampler)
