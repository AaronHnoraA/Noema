...
(defun magent-memory--excluded-path-p (path &optional root)
  "Return non-nil when PATH should be excluded by default.
When ROOT is non-nil, match directory exclusions against PATH relative to
ROOT.  This prevents a configuration rooted below a system directory named
`var' or `cache' from being excluded in its entirety."
  (let* ((expanded (expand-file-name path))
         (relative (and root (file-relative-name expanded
                                                 (expand-file-name root))))
         (candidate (if (and relative
                             (not (string-prefix-p "../" relative)))
                        (concat "/" relative)
                      expanded))
         (normalized (replace-regexp-in-string "\\\\" "/" candidate)))
...
path even when its contents are not read."
  (let ((name (file-name-nondirectory file)))
    (and (magent-memory--regular-readable-file-p file)
         (not (magent-memory--excluded-path-p file root))
        ...
        ((walk (path)
           (cond
            ...
            ((magent-memory--sensitive-path-p path)
             (push path sensitive))
            ((magent-memory--excluded-path-p path root)
             (push path excluded))
            ((file-directory-p path)
    ...
    (let ((state
           (magent-memory-run
            ...
            :notify-fn (lambda (message)
                         (message "%s" message)
                         (magent-internal-command-notify context message))
            :on-complete (lambda (status message)
                           (magent-internal-command-complete context status message))
