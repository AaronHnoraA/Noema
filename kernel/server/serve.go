...
import (
	...
	"github.com/aaronhe/noema/kernel/mcp"
	...
	util.WebSocketServer.HandleConnect(func(s *melody.Session) {
			...
			if err != nil {
				...
				if nil == val {
					...
					if err != nil {
						...
						authOk = model.IsAccessCodeSessionAuthenticated(workspaceSess)
					}
				}
			}
		}
		...
		if !authOk {
			if token := model.ParseXAuthToken(s.Request); token != nil {
				authOk = token.Valid && model.RoleAdministrator == model.GetClaimRole(model.GetTokenClaims(token))
			}
		}
...
func jwtMiddleware(c *gin.Context) {
	...
	c.Set(model.RoleContextKey, model.RoleUnauthenticated)
	c.Next()
}
