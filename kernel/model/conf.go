...
import (
	...
	"github.com/aaronhe/noema/kernel/heif"
...
func InitConf() {
	...
	if util.ContainerDocker == util.Container && Conf.AccessAuthCode == "" && !util.SiYuanAccessAuthCodeBypass {
		fmt.Println("the access authorization code must be set when deploying via Docker")
		fmt.Println("set SIYUAN_ACCESS_AUTH_CODE, or explicitly set SIYUAN_ACCESS_AUTH_CODE_BYPASS=true")
		logging.LogErrorf("Docker access authentication configuration is invalid: access authorization code is empty")
