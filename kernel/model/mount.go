...
func CreateBox(name string) (id string, err error) {
	return createBox(name, true, conf.BoxKindSy)
}

// CreateMarkdownBox 创建一个 markdown box：.md 文件是真相源，跳过 initializeBoxDoc
// （ensureBoxDoc0 是 .sy 嵌套文档树的"笔记本自身文档"概念，不适用——见
// mount.go 里 mountBox 对 markdown box 同样跳过 EnsureBoxDoc/ListDocTree 的注释）。
func CreateMarkdownBox(name string) (id string, err error) {
	return createBox(name, false, conf.BoxKindMarkdown)
}

func createBox(name string, initializeBoxDoc bool, kind string) (id string, err error) {
