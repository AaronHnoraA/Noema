...
func LoadTreeWithFix(boxID, p string, luteEngine *lute.Lute) (ret *parse.Tree, needFix bool, err error) {
	...
	if isMarkdownBox(boxID) {
		// markdown box：.md 字节是真相源，没有 JSON 订正/加密/rootID 前置缓存这些 .sy 专属概念。
		ret, err = loadMarkdownTree(boxID, p, luteEngine)
		return
	}
...
func loadMarkdownTree(boxID, p string, luteEngine *lute.Lute) (ret *parse.Tree, err error) {
	...
	if nil != err {
		if !os.IsNotExist(err) {
			logging.LogErrorf("load markdown tree [%s] failed: %s", p, err)
		}
		return
	}
