...
func setAccessAuthCode(c *gin.Context) {
	...
	if aac == "" && !model.IsLocalRequest(c) && !util.SiYuanAccessAuthCodeBypass {
		ret.Code = -1
		ret.Msg = model.Conf.Language(369)
		logging.LogWarnf("reject clearing the last usable access authentication method [ip=%s]", c.ClientIP())
		return
	}
