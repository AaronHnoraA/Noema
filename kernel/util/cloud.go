...
const (
	...
	// Noema fork: the bazaar marketplace's network catalog is disabled —
	// Noema ships one curated theme/icon set instead of live-downloading
	// community packages. These point at a reserved, unroutable address
	// (RFC 5737 TEST-NET-1) so any lingering fetch attempt fails fast
	// (connection refused/timeout) rather than actually reaching a
	// b3log-operated server; every call site already has to tolerate the
	// bazaar being offline, since that's a normal state for this app even
	// upstream.
	BazaarStatServer = "http://192.0.2.1"
	BazaarOSSServer  = "http://192.0.2.1"
)
