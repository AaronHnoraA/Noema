...
- **D-037 第二轮（2026-09-16，当前工作树）**：JuText `C-c j p` / 右键 Preview context 调 `run:context-preview` 预演路由与上下文（不冻结、不 compaction、不派发）；Emacs rollover 比例随 Run/预演传给 Node；Go `LatestWorkNodeActivity` + events `latestPerWorkNode` 修正 Graph 取到最早 1000 条事件导致的陈旧活动时间；Graph 按 WorkNode 索引 Run/事件；空闲会话预览共享 2 秒快照，Run 与名字变更即失效；多个 lineage 父节点 agent 不一致时由文档 `default_agent` 决定。证据：Go research/api 全过，Node research 聚焦套件 70 项与 `tsc --noEmit` 全过，AaronEmacs `make research-test` 为 1+38+8+144+74 全过；完整 `npm test` 为 2389 passed / 16 skipped，唯一失败是 `drag-select-cost` 5 MB 帧耗时用例，单独重跑 3/3 通过。
- **D-036 深度优化（2026-09-16，当前工作树）**：Graph `f` 改为以节点为根（`^` 上移根、`Deeper branches` 下钻、镜头外的选中/同步/新建自动放宽，删除 `focus-siblings` 设置）；MarkMind 式结构导航 `H`/`L`/`{`/`}` 与 `/` 按标题跳转；`@@ctx(lineage:N)`、`@@ctx(none)`；推导出 lineage 的 Run 自动附加直接父节点 output，自动上下文只用显式条目剩余的 64 KiB 预算（截断/省略写入 `context_omitted`）；Go `latestPerWorkNode` 让路由看到 1000 个 Run 之外的会话，推导按 WorkNode 建索引；Go `run/handoff` 只读状态事件取 Handoff（新索引 `events(run_id,type)`），文件快照并发 `stat`；Sessions 管理器 Context 列与 `c` compaction；Node 文件基线清理与 TTL、Emacs ledger 的 Run-id 键回收与 turn 上限轮换；`MaintainCache` 清理 7 天前已完成且有 Transcript 的 Run 的流式 segment。证据：Go research/api/mcp 全过，Node research 聚焦 4 文件与 `tsc --noEmit` 全过，AaronEmacs `make research-test` 为 1+38+8+143+74 全过；完整 `npm test` 为 2376 passed / 16 skipped，唯一失败是 `optimal-linebreak` 的 40ms 负载型计时用例，单独重跑 8/8 通过；严格字节编译无警告，两个仓库 `git diff --check` 干净。尚未做真实 Emacs GUI 走查。
