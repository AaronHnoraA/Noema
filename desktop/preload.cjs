...
contextBridge.exposeInMainWorld("noemaDesktop", {
  ...
  closeWindow() {
    return ipcRenderer.invoke("noema:close-window");
  },
  startWindowDrag() {
    // Electron uses -webkit-app-region: drag on the shared title bar.
  ...
  notifyAppConfigChanged(revision) {
    ipcRenderer.send("noema:broadcast-app-config", String(revision || ""));
  },
  reportSmoke(report) {
    return ipcRenderer.invoke("noema:desktop-smoke-report", report || {});
  },
  onCommand(callback) {
