...
// Suites run with an isolated global capability scope (see
// tests/setup/global-capability-scope.ts). A test that builds its own fake
// home must start from an empty environment as well, because that isolation
// variable — not `userHome` — would otherwise decide where global scope reads.
function resolveCapabilities(options: Parameters<typeof resolveProjectCapabilities>[0]) {
  return resolveProjectCapabilities({ environment: {}, ...options });
}
...
describe("Noema capability resolution", () => {
  test("unified diffs preserve global sources, apply with patch and freeze at run resolution", async () => withTree(async ({ root, home, builtin }) => {
    ...
    const project = await resolveCapabilities({ ...options, requestedSkills: ["proof"], includeContent: true });
    ...
    expect((await resolveCapabilities({ ...options, scope: "global", includeContent: true })).skills[0].effective.content).toBe(original);
    expect(await readFile(join(globals, "proof", "SKILL.md"), "utf8")).toBe(original);
    await writeFile(join(globals, "proof", "SKILL.md"), original + "\nUpstream changed\n");
    expect((await resolveCapabilities(options)).skills[0].validation.valid).toBe(false);
    await writeFile(join(globals, "proof", "SKILL.md"), original);
    ...
    expect((await resolveCapabilities({ ...options, includeContent: true })).skills[0].effective.content).toBe(original);
  ...
  test("explicit Patch upgrades a legacy full-document override without losing its edits", async () => withTree(async ({ root, home, builtin }) => {
    ...
    expect((await resolveCapabilities({ ...options, includeContent: true })).skills[0].effective.content).toBe(edited);
  }));
  ...
  test("copying global Skills creates independent files and resources, including symlinks", async () => withTree(async ({ root, home, builtin }) => {
    ...
    const local = (await resolveCapabilities({ ...options, includeContent: true })).skills.find((item) => item.id === "proof")!;
  ...
  test("invalid patch files fail closed, and symlinks cannot read outside the project", async () => withTree(async ({ root, home, builtin }) => {
    ...
    const result = await resolveCapabilities(options);
  ...
  test("global management needs no project and writes only the global library", async () => withTree(async ({ root, home, builtin }) => {
    ...
    const result = await resolveCapabilities({ ...options, root: "/a/nonexistent/project" });
  ...
  test("reads linked native libraries without copying configs or inheriting automatic MCP startup", async () => withTree(async ({ root, home, builtin }) => {
    ...
    expect((await resolveCapabilities(options)).active.mcps).toContain("example");
    expect(await readFile(native, "utf8")).toBe(original);
    const selected = resolvedSkillsForRun(await resolveCapabilities({ ...options, requestedSkills: ["linked-proof"], includeContent: true }));
  ...
  test("global config and Skill roots can be overridden independently", async () => withTree(async ({ root, home, builtin }) => {
    ...
    const resolution = await resolveCapabilities({ root, builtinSkillDirectory: builtin,
  ...
  test("a project definition wins deterministically and records the shadowed source", async () => withTree(async ({ root, home, builtin }) => {
    ...
    const resolution = await resolveCapabilities({ root, userHome: home, builtinSkillDirectory: builtin, includeContent: true });
  ...
  test("rejects malformed project configuration with source and field", async () => withTree(async ({ root, home, builtin }) => {
    ...
    await expect(resolveCapabilities({ root, userHome: home, builtinSkillDirectory: builtin }))
  ...
  test("mutates only the project selection/patch document and resolves the result", async () => withTree(async ({ root, home, builtin }) => {
    ...
    const disabled = await resolveCapabilities({ root, userHome: home, builtinSkillDirectory: builtin });
